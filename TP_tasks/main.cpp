...
bool select_product() {
	if (products.size() == 0) {
		...
		while (1) {
			...
			else if (check_prod_numb(number_product) == true) 	{
				Check* check = new Check;
				check->input_purchase(products.at(number_product - 1));
				purchase.push_back(check);
			} else cout << "В магазине нет такого продукта. " << endl;
		} return true;
	}	
}
...
bool check_prod_numb(int number_products) {
	if ((number_products <= products.size())&& (number_products>0)) return true;
	else return false;
}
...
int input_number() {
	...
	while (1) {
		...
		if (intSigned(number_s) == true) {
			...
			return number;
		} else 	cout << "Данный пункт отсутствует. Повторите ввод." << endl << ">> ";
	}
}

//проверки на ввод
bool intSigned(const string& s) {
	if (s.find_first_not_of("0123456789", 0) == string::npos) return true;
	else return false;
}
...
bool stringSigned(const string& s) {
	if (s.find_first_not_of(" qwertyuioplkjhgfdsamnbvcxzPOIUYTREWQLKJHGFDSAMNBVCXZ", 0) == string::npos) return true;
	else return false;
}